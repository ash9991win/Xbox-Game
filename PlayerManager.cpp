...
void PlayerManager::Update(DWORD newMS)
{
	...
	{
		currentTime = 0;
		//	mPosition.rotateOnPivot(rotationSpeed * (newMS/1000.0f),&collider.mCenter);
		//Vector2 relativePos = mPosition.findDiff(rotationPoint);
	 //	mPosition.rotateOnPivot((rotationAngle) , &rotationPoint);
		//if (direction)
		//{
		//	mPosition.rotateOnPivot((rotationSpeed * newMS / 1000.0f), &rotationPoint);
		//}
		//else
		//{
		//	mPosition.rotateOnPivot((-rotationSpeed * newMS / 1000.0f), &rotationPoint);
		...
		//float32_t tempCos = cosf(rotationSpeed * newMS / 1000.0f);
		//float32_t tempSin = sinf(rotationSpeed * newMS/1000.0f);
		//float32_t tempX;
		//float32_t tempY;
		//mPosition.x -= rotationPoint.x;
		//mPosition.y -= rotationPoint.y;
		//tempX = mPosition.x * tempCos + mPosition.y * tempSin;
		//tempY = mPosition.y * tempCos - mPosition.x * tempSin;
		//mPosition.x = tempX;
		//mPosition.y = tempY;
		//mPosition.x += rotationPoint.x;
		//mPosition.y += rotationPoint.y;
	//	rotationAngle +=  newMS / (float)1000.0f;
	 //  rotationSpeed += newMS / 100000.0f;
		//mVelocity.x = mVelocity.y = 0;
		
		
	////	updatedPosition = mPosition;
	//	mPosition.rotateOnPivot(rotationAngle, &rotationPoint);
		if (!direction)
		{
	...
	case JUMPING_STATE:
	{
		if (isMegajump)
		{
			...
			mPosition.y += 20 *mVelocity.y * currentTime /*+ (0.5 *GRAVITY *currentTime * currentTime )*/;
		...
		else
		{
			...
			mPosition.y += 10* mVelocity.y * currentTime /*+ (0.5 *GRAVITY *currentTime * currentTime)*/;
...
void PlayerManager::Render()
{
	...
	newPosition.y -= cameraPosition.y;
		...
		//playerRenderer->DrawSprite(newPosition, mWidth, mHeight, currentTexture, -rotationAngle, rotationPoint.findDiff(cameraPosition));

		//playerRenderer->DrawSprite(newPosition, mWidth, mHeight, currentTexture, -rotationAngle, collider.mCenter.findDiff(cameraPosition));
