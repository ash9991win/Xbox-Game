...
void CGame::UpdateFrame(DWORD milliseconds)			
{
	...
	//	SlideManager::getInstance()->init();
		...
		//LevelLoader::getInstance()->setSlideList(SlideManager::getInstance()->getSlideList());
	...
	case LOAD_LEVEL_STATE:
		if (PrincessManager::getInstance()->isAnimationOver)
		{
			...
			//SlideManager::getInstance()->init();
			PlayerManager::getInstance()->Init();
			//ScoreManagerC::GetInstance()->init(scorePosRatio, scoreSizeRatio);
			PrincessManager::getInstance()->init();
			Collectible::getInstance()->init();
			LevelTextManager::getInstance()->init();
			FontManagerC::GetInstance()->init();
...
void CGame::DrawScene(void)											
{
	...
	case PLAY_STATE:
		...
		//SlideManager::getInstance()->render();
...
{
	//printf("COLLODED %d \n", PlayerManager::getInstance()->getCurrentState());
	
	ListIterator<Discs*> *discIterator = DiscManagerC::getInstance()->getDiscs();
	bool checkForCollision = true;
	discIterator->Start();
	//if (!PlayerManager::getInstance()->isRotating()/* && !PlayerManager::getInstance()->isJumping()*/)
	//{
		while (!discIterator->hasReachedEnd() )
		{
			...
			if (CollisionManager::getInstance()->checkForCollision(discIterator->data()->collider, PlayerManager::getInstance()->getCollider()))
				{
			/*if (CollisionManager::getInstance()->checkForCollision(PlayerManager::getInstance()->getCollider(), discIterator->data()->collider))
			{*/
					if (discIterator->data()->discType == JUMPABLE_DISC)
					{
						if (PlayerManager::getInstance()->getRotatingPoint() != discIterator->data()->center)
						{
							...
							//PlayerManager::getInstance()->setRotationAngle(discIterator->data()->currentAngle);
				...
				{
				//	discIterator->data()->canSeePlayer = false;
				}
				discIterator->forth();
...
void CGame::checkForInput()
{
	...
	//if (InputManagerC::GetInstance()->getButtonPressed(GAME_KEY_JUMP)/* && PlayerManager::getInstance()->isRotating()*/)
	/*{
		if (!PlayerManager::getInstance()->isJumping())
		{
			PlayerManager::getInstance()->changeCurrentState(JUMPING_STATE);
			printf("JUMPING AND CHANGING STATE");
		}
	}*/
...
void CGame::shutdown()
{
	//EntityManagerC::GetInstance()->shutdown();
...
{
	/*delete EntityManagerC::GetInstance();
